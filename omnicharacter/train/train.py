...
from omnicharacter import conversation as conversation_lib
from omnicharacter.constants import (DEFAULT_IMAGE_TOKEN, DEFAULT_SPEECH_TOKEN, IGNORE_INDEX, IMAGE_TOKEN_INDEX,SPEECH_TOKEN_INDEX)
from omnicharacter.mm_utils import process_anyres_image, tokenizer_image_token
from omnicharacter.model import *
from omnicharacter.train.llava_trainer import LLaVATrainer
...
def preprocess_qwen_2(sources, tokenizer: transformers.PreTrainedTokenizer, 
    ...
    for i, source in enumerate(sources):
        ...
        if source[0]["from"] == "system":
            ...
            system_message = "You are a helpful language and speech assistant. You are able to understand the text content that the user provides, and assist the user with a variety of tasks using natural language or speech."
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'speech' in sources[0]:
            ...
            num_speech_rounds = len(sources[0]["speech"])
            speech = torch.zeros((num_speech_rounds, 3000, 128), dtype=torch.float32)
            valid_idx = 0  # To keep track of valid entries
            for idx, (a, b) in enumerate(zip([i for i in sources[0]["conversations"][1:] if i['from'] == 'human'], sources[0]["speech"])):
                if (
                    random.random() < self.speech_prob
                    and 'none' not in b
                    and os.path.exists(osp.join(speech_folder, b))
                ):
                    round_speech = whisper.load_audio(osp.join(speech_folder, b))
                    ...
                    # round_speech = torch.from_numpy(np.load(osp.join("/mnt/workspace/haonan/code/llama_audio_character/datasets/audio_array", osp.splitext(b)[0] + ".npy")))
                    speech[valid_idx] = round_speech  #q Populate only the valid positions
                    valid_idx += 1  # Move to the next valid position
                    # print("successfully load the audio files！")
